...
namespace SchoolAPI.Infrastructure
{
    public class ConsulHostedService : IHostedService
    {
        ...
        public async Task StartAsync(CancellationToken cancellationToken)
        {
            ...
            var address = _consulConfig.Value.HostAddress ?? addresses.Addresses.First();            

            var uri = new Uri(address);
            //_registrationID = $"{_consulConfig.Value.ServiceID}-{uri.Port}";
            _registrationID = $"{_consulConfig.Value.ServiceID}-{Environment.GetEnvironmentVariable("HOSTNAME")}-{uri.Port}";

            var healthcheck = $"{uri.Scheme}://{uri.Host}:{uri.Port}/api/health/status";
